...
abstract interface class AnimalStoryGenerator {
...
class AnimalStoryService implements AnimalStoryGenerator {
  ...
  Future<AnimalStory> create({
    ...
    final labels = {
      for (final dimension in schema.dimensions)
        dimension.id: {
          for (final option in dimension.options) option.value: option.label,
        },
    };
    ...
    final investigation = {'status': 'completed', 'observations': observations};
    ...
    return AnimalStory.fromJson(value);
  }
}
