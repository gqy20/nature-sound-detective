...
1. 模型阶段结果分散在 Qwen、BirdNET、非鸟分类头及融合层中；
...
- 后续大模型二次推理应扩展共享领域层，而不是直接写入界面或 CLI。
...
当前状态机只实现决赛最小的一轮交互，不声称已经完成 Qwen 二次排序。下一阶段可在保持 Schema 兼容的前提下增加 `agent_reviewing` 和第二轮问题。
...
| 模式 | 行为 | 远程调用 |
|---|---|---|
| `acoustic` | BirdNET + 非鸟分类头 + 共享融合 | 无 |
| `qwen` | Qwen 声景分析 + 共享融合 | 有 |
| `full` | 与本地完整版一致的完整分析管线 | 有 |
...
运行包默认不复制原始录音，不保存API Key。后续扩展Prompt调试时，必须先建立更严格的请求/响应脱敏策略。
