...
退出条件：固定样例能够稳定复现；声音分析不依赖通用大模型；测试过程不会触发MiniMax或视频API。
...
| API 费用失控 | 自动测试触发MiniMax/视频接口 | 默认mock，显式开关才允许live |
...
| 密钥泄露 | 把MiniMax或Wan密钥打入APK/IPA | 第三方生成API只允许服务器调用 |
...
4. 验证分析链路不调用通用大模型，MiniMax和视频接口默认不会被自动调用；
